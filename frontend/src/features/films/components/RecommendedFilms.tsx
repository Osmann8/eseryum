...
import { FilmListItem } from "@/features/films/components/FilmListItem";
import { MatchBadge } from "@/features/films/components/MatchBadge";
...
export function RecommendedFilms({ films }: { films: RecommendedFilm[] }) {
  ...
  return (
    <section className="mb-9">
      ...
      {films.length === 0 && <p className="text-sm text-ink-muted">{t("empty")}</p>}

      {films.length > 0 && view === "LIST" && (
        <ul className="flex flex-col gap-2">
          {films.map((film) => (
            <li key={film.id}>
              <FilmListItem
                film={film}
                trailing={<MatchBadge score={film.matchScore} />}
              />
            </li>
          ))}
        </ul>
      ...
      {films.length > 0 && view === "GRID" && (
        <div className="relative">
          ...
          >
            {films.map((film) => (
              <li key={film.id} className="w-[152px] shrink-0">
                <FilmCard
                  film={film}
                  badge={<MatchBadge score={film.matchScore} />}
                />
              </li>
            ))}
          </ul>
